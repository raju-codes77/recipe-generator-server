...
export class CommunityService {
  ...
  async listPosts(
     ...
     *
     * Example:
     *   2026-09-02T10:20:30.000Z|cm123abc
     */
    ...
    if (options.cursor) {
      ...
      if (separatorIndex > 0) {
        const datePart = options.cursor.slice(0, separatorIndex);
        const idPart = options.cursor.slice(separatorIndex + 1);
    ...
    /*
     * Fetch one extra post so we can determine hasMore.
     *
     * We intentionally use take + 1 instead of a COUNT query.
     * COUNT on every pagination request would add unnecessary DB work.
     */
    ...
    const pagePosts = posts.slice(0, take);
    ...
    /*
     * All independent queries are executed concurrently.
     *
     * Before:
     *   users
     *   ↓
     ...
     *   viewer data ───┘
     *
     * This dramatically reduces request latency.
     */
    ...
    const [
      ...
      // Viewer made-it
      viewerId
        ? prisma.communityMadeIt.findMany({
            where: {
              postId: {
                in: postIds,
              },
    ...
    /*
     * Convert DB results into Maps/Sets.
     * This keeps the final mapping O(n) and avoids repeated searches.
     */
    ...
    const madeItsByPostId = new Map(
      madeItCounts.map((item) => [
        item.postId,
        item._count._all,
      ]),
    );
    ...
    const viewerLikedPostIds = new Set(
      viewerReactions.map((item) => item.postId),
    );

    const viewerSavedPostIds = new Set(
      viewerSaves.map((item) => item.postId),
    );

    const viewerMadeItPostIds = new Set(
      viewerMadeIts.map((item) => item.postId),
    );

    /*
     * Count recipes per author only within the current page.
     *
     * This preserves the existing response behavior.
     */
    const recipesByAuthorId = new Map<string, number>();

    for (const post of pagePosts) {
      if (post.recipeId) {
        recipesByAuthorId.set(
          post.authorId,
          (recipesByAuthorId.get(post.authorId) || 0) + 1,
        );
      }
    }
    ...
    const result = pagePosts.map((post) => {
      ...
      return {
        ...
        author: {
          ...
          username: (
            author?.email.split("@")[0] || "community_cook"
          ).replace(/[^a-zA-Z0-9_]/g, "_"),
          avatar: author?.image || "",
          role: "user" as const,
          followersCount:
            followersByUserId.get(post.authorId) || 0,
          isFollowing: following.has(post.authorId),
          recipesCount:
            recipesByAuthorId.get(post.authorId) || 0,
        },
        ...
        recipe: recipe
          ? {
              ...
              difficulty: (
                (
                  recipe.instructions as unknown as {
                    difficulty?: string;
                  } | null
                )?.difficulty || "Easy"
              ) as "Easy" | "Medium" | "Hard",
              ...
              prepTimeMinutes: Math.max(
                0,
                Math.floor(recipe.time / 3),
              ),

              cookTimeMinutes: Math.max(
                0,
                recipe.time - Math.floor(recipe.time / 3),
              ),

              servings: 2,

              dietaryTags: recipe.category
                ? [recipe.category]
                : ["Community Recipe"],

              ingredients: recipe.ingredients.map((item) => ({
                name: item.name,
                amount: item.measure || "As needed",
              })),
        ...
        rating: {
          ...
          presentation: average(
            review?._avg.presentationRating,
          ),
          totalReviews: review?._count._all || 0,
        },

        likesCount:
          reactionsByPostId.get(post.id) || 0,

        isLiked:
          viewerLikedPostIds.has(post.id),

        savesCount:
          savesByPostId.get(post.id) || 0,

        isSaved:
          viewerSavedPostIds.has(post.id),

        commentsCount:
          commentsByPostId.get(post.id) || 0,
        ...
        madeItCount:
          madeItsByPostId.get(post.id) || 0,

        hasMadeIt:
          viewerMadeItPostIds.has(post.id),
    ...
    /*
     * Generate cursor from the LAST item of the current page.
     *
     * The frontend/controller can send this cursor on the next request.
     */
  ...
  async getPublicProfile(
    ...
    if (!user) {
      throw Object.assign(
        new Error("Community user not found"),
        { statusCode: 404 },
      );
    }

    const postsResult = await this.listPosts(viewerId, {
      authorId: userId,
      take: 50,
    });
    ...
    const followersCount =
      await prisma.communityFollow.count({
        where: {
          followingId: userId,
        },
      });
  ...
  async getPostInteractions(
    ...
    const commentsTake = Math.min(
      Math.max(options.commentsTake ?? 8, 0),
      20,
    );

    const commentsSkip = Math.max(
      options.commentsSkip ?? 0,
      0,
    );

    const reviewsTake = Math.min(
      Math.max(options.reviewsTake ?? 5, 0),
      10,
    );

    const reviewsSkip = Math.max(
      options.reviewsSkip ?? 0,
      0,
    );
    ...
    return {
      ...
      reviews: reviews.map((review) => {
        ...
        return {
          ...
          flavorRating:
            review.flavorRating || undefined,
          easeRating:
            review.easeRating || undefined,
          presentationRating:
            review.presentationRating || undefined,
          comment: review.comment,
          cookingTips:
            review.cookingTips || undefined,
  ...
  async createPost(
    user: AuthenticatedCommunityUser,
          ...
          const recipe = await tx.recipe.create({
            data: {
              ...
              title:
                ...
                "Community Recipe",
              category:
                input.recipe.dietaryTags?.join(
                  ", ",
                ) || "Community Recipe",
              cuisine: input.recipe.cuisine,
              time:
                Number(
                  input.recipe.prepTimeMinutes || 0,
                ) +
                Number(
                  input.recipe.cookTimeMinutes || 0,
                ),
              calories: Number(
                input.recipe.nutrition?.calories || 0,
              ),
    ...
    return {
      ...
      recipe: input.recipe
        ? {
            ...
            servings: Number(
              input.recipe.servings || 2,
            ),

            dietaryTags:
              input.recipe.dietaryTags || [],
      ...
      isChallengeEntry:
        createdPost.isChallengeEntry,

      challengeName:
        createdPost.challengeName ||
        undefined,
  ...
  async deletePost(
    ...
    await prisma.$transaction([
      ...
      prisma.communityReview.deleteMany({
        where: { postId },
      }),
      prisma.communitySavedPost.deleteMany({
        where: { postId },
      }),
      prisma.communityMadeIt.deleteMany({
        where: { postId },
      }),
  ...
  async toggleRecord(
    ...
    if (model === "reaction") {
      ...
      const existing =
        await prisma.communityReaction.findUnique({
          where,
        });
      ...
      if (!existing) {
        await this.notifyPostOwner(
          postId,
          userId,
          "LIKE",
          "liked your Community recipe",
        );
      }
    ...
    const existing =
      await prisma.communityMadeIt.findUnique({
        where,
      });

    if (existing) {
      await prisma.communityMadeIt.delete({
        where,
      });
    } else {
      await prisma.communityMadeIt.create({
        data: {
          postId,
          userId,
        },
  ...
  async saveReview(
    ...
    await this.notifyPostOwner(
      ...
      userId,
      "REVIEW",
      "reviewed your Community recipe",
    );
  ...
  async listCollections(userId: string) {
    const collections =
      await prisma.communityCollection.findMany({
        where: { userId },
        orderBy: { createdAt: "desc" },
      });
  ...
  createCollection(
    ...
    return prisma.communityCollection.create({
      data: {
        userId,
        name,
        description,
  ...
  async savePost(
    ...
    const existing =
      await prisma.communitySavedPost.findUnique(
        {
          where: {
            postId_userId: {
              postId,
  ...
  async listStories(authorId?: string) {
    const stories =
      await prisma.communityStory.findMany({
        where: {
          expiresAt: {
            gt: new Date(),
          },
  ...
  async deleteStory(
    ...
    const story =
      await prisma.communityStory.findUnique(
        {
          where: { id: storyId },
        },
      );
  ...
  async listNotifications(
    ...
    const notifications =
      await prisma.communityNotification.findMany(
        {
          where: { userId },
          orderBy: {
            createdAt: "desc",
  ...
  async listMessages(
    ...
    const conversation =
      await prisma.communityConversation.findUnique(
        {
          where: { pairKey },
        },
      );
